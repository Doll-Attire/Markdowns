...
>寻址:	主机向总线发送从机地址,从主机开始向SDA写入数据的瞬间,SCL激活~也即被拉低~,然后开始锯齿状传输时钟信号.	常用是7位地址.剩下的一位^R/W#,井号代表低电压有效,即0写1读^代表数据传输的方向,随后主机释放SDA等待从机应答,从机把SDA拉低发送应答信号ACK(acknowledge)
>
>在从机应答过程中,可能无法ACK,zhechengwei
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEzNzk1NjIwMywtOTYxNzg2NDgyLC0xMz
