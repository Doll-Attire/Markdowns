...
检测GPIO电平变化，向NVIC发出中断申请，等待其向CPU发出中断程序请求。

支持所有GPIO口，但是PA0，PB0这样Pin相同的不能同时中断。（因为中断引脚选择只有16Pin）

触发响应方式：中断响应、事件响应（触发事件，中断事件流向其他外设而非CPU）

通道数：16GPIO_Pin + 4个其他的（）




<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEzNTI2NTI1MSwxMjA1MDMwMDUyXX0=
-->